...
export default function EditTenderPage() {
  ...
  useEffect(() => {
    ...
    supabase
      ...
      .then(({ data, error }) => {
        ...
        const t = data as Tender
        setTitle(t.title)
        setReferenceNumber(t.reference_number)
        setSummary(t.summary)
        setDescription(t.description)
        setClosingDate(t.closing_date.slice(0, 16))
        setStatus(t.status)
        setExistingDocUrl(t.document_url)
        setFetching(false)
      })
  }, [id])
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!supabase || !id) return
    setLoading(true)
    let documentUrl: string | null = existingDocUrl
  ...
  return (
    <div className="container mx-auto px-4 py-8 max-w-2xl">
      ...
      <h1 className="text-2xl font-bold text-gray-900 mb-8">Edit Tender</h1>
      <form onSubmit={handleSubmit} className="space-y-6 bg-white rounded-xl border border-gray-200 p-6 md:p-8">
        ...
        <div>
          ...
          <Input value={referenceNumber} onChange={(e) => setReferenceNumber(e.target.value)} className={inputClass} required />
        </div>
        <div>
          <Label>Summary *</Label>
        ...
        <div>
          ...
          {existingDocUrl && <p className="text-sm text-gray-500 mb-1">Current file attached. Upload a new file to replace.</p>}
          <Input
            type="file"
